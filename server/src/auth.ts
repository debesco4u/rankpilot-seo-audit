...
import db from './db';
import { sendPasswordResetToUser } from './email';
...
router.post('/forgot-password', async (req: Request, res: Response) => {
  try {
    ...
    const sent = await sendPasswordResetToUser(user.email, token);
    if (sent) res.json({ message: 'Password reset link sent! Check your email inbox (and spam folder) for the reset link.' });
    else res.status(500).json({ error: 'Failed to send reset email. Please try again or contact seo@dabisoftsolutions.com for help.' });
  } catch (err: any) { res.status(500).json({ error: err.message }); }
});
